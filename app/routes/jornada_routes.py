...
from app.schemas.jornada_schema import JornadaCreateDTO, JornadaUpdateDTO, JornadaResponseDTO, HorarioJornadaDTO
...
def criar_jornada(dto: JornadaCreateDTO, session: Session = Depends(pegar_sessao)):
    return criar_jornada_service(session, dto)
...
# ----------------- Rotas Horário -----------------
@jornada_router.post("/{jornada_id}/horarios", response_model=HorarioJornadaDTO, status_code=201)
def criar_horario(jornada_id: UUID, dto: HorarioJornadaDTO, session: Session = Depends(pegar_sessao)):
    return criar_horario_service(session, jornada_id, dto)

@jornada_router.get("/{jornada_id}/horarios", response_model=List[HorarioJornadaDTO])
...
@jornada_router.put("/horarios/{horario_id}", response_model=HorarioJornadaDTO)
def atualizar_horario(horario_id: UUID, dto: HorarioJornadaDTO, session: Session = Depends(pegar_sessao)):
